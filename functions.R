...
FitSamplePredict = function(nLoc, theIntercept, dataList, parameters, random,  flag2, predCoords, mesh.s, u.sim, covariatesAtPred,
  ...
  log_tau_draws  <- t.draws[parnames == 'log_tau',]
  log_kappa_draws  <- t.draws[parnames == 'log_kappa',]
  
  sp_range_draws = sqrt(8.0)/exp(log_kappa_draws)
  sp_sigma_draws = 1.0 / sqrt(4.0 * 3.14159265359 *
                                       exp(2.0 * log_tau_draws) * exp(2.0 * log_kappa_draws))
  ...
  # 
  # 
  SampledParameters = cbind(median = c(sp_range = median(sp_range_draws), sp_sigma = median(sp_sigma_draws),
                                       intercept = median(intercept), beta_distRiversLakes = median(beta_distRiversLakes), beta_accessCities = median(beta_accessCities),
                                       beta_elevation = median(beta_elevation), beta_population = median(beta_population), beta_urbanicity = median(beta_urbanicity)),
                            lower = c(quantile(sp_range_draws, probs = 0.025), quantile(sp_sigma_draws, probs = 0.025),
                                      quantile(intercept, probs = 0.025), quantile(beta_distRiversLakes, probs = 0.025), quantile(beta_accessCities, probs = 0.025),
                                      quantile(beta_elevation, probs = 0.025), quantile(beta_population, probs = 0.025), quantile(beta_urbanicity, probs = 0.025)),
                            upper = c(quantile(sp_range_draws, probs = 0.975), quantile(sp_sigma_draws, probs = 0.975),
                                      quantile(intercept, probs = 0.975), quantile(beta_distRiversLakes, probs = 0.975), quantile(beta_accessCities, probs = 0.975),
                                      quantile(beta_elevation, probs = 0.975), quantile(beta_population, probs = 0.975), quantile(beta_urbanicity, probs = 0.975)),
                            length = c((quantile(sp_range_draws, probs = 0.975)- c(quantile(sp_range_draws, probs = 0.025))), (quantile(sp_sigma_draws, probs = 0.975)- c(quantile(sp_sigma_draws, probs = 0.025))),
                              (quantile(intercept, probs = 0.975)- c(quantile(intercept, probs = 0.025))), (quantile(beta_distRiversLakes, probs = 0.975)- quantile(beta_distRiversLakes, probs = 0.025)),
